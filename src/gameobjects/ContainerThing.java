...
public class ContainerThing extends ThingHolder implements java.io.Serializable {
    ...
    public String open() {
        ...
        if (!openable) {
            ...
            // + article(d) + " " + d
            char caractere_final = this.getName().charAt(this.getName().length()-1);
        
            if (caractere_final == 'a')  {
                    s = "Não consigo abrir a " +getDescription();
            }
            else  {
                s = "Não consigo abrir o " +getDescription();
            }
        } else {
            if (isopen) {
                // s = "The " + getDescription() + " is already open.";
                s = article(getName()) + " " + getDescription() + " já está abert"+article(getName());
                
            } else {
                isopen = true;
                // s = "You open the " + getDescription();
                s = "Você abriu " + article(getName()) + " " + getDescription();
    ...
    public String describe() {
        ...
        if (openable) {
            if (isopen) {
                // s += " (open)";
                s += " (abert" + article(this.getName())+")";
                
            } else {
                // s += " (closed)";
                s += " (fechad" + article(this.getName())+")";
        ...
        if (isopen) {
            if (getThings().size() > 0) {
                ...
                // s += "\nTem algo dentro del"+ article(this.getName())+".";
                s += "\nTem algo dentro.";
            }
        }
        return s;
