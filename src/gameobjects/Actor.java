...
public class Actor extends ThingHolder implements java.io.Serializable {
    ...
    public String inventory() {
        ...
        return "Você possui no momento: " + s;
    }
    ...
    public String unlockWith(NounPhrase np, NounPhrase np2) {
        ...
        // s = ListTool.oneThingInList(thingToOpenList, np.phrase(), "destranque");
        if (s.isEmpty()) {
            s = ListTool.oneThingInList(thingToUseList, np2.phrase(), "use", true);
            if (s.isEmpty()) {
                t_th = thingToOpenList.get(0);
    ...
    public String lockOb(NounPhrase np) {
        ...
        if (s.isEmpty()) {
            ...
            s = "Você precisa dizer com o que você deseja trancar ele.";
        }
        return s;
    }
    ...
    public String unlockOb(NounPhrase np) {
        ...
        if (s.isEmpty()) {
            ...
            s = "Você precisa dizer com o que você deseja abrir ele.";
        }
        return s;
    }
    ...
    public String lookIn(NounPhrase np) {
        ...
        if (s.isEmpty()) {
            ...
            if (container == null) {
                ...
                s = "Você não pode olhar dentro d"+article(t.getName())+" " + t.getDescription() + ".";
            } else {
                if (container.isOpen()) {
                    s = container.describeThings();
                    if (s.isEmpty()) {
                        // s = "There is nothing in the " + container.getDescription();
                        s = "Não vejo nada dentro.";
                    } else {
                        s = article(t.getName()) + " "  + container.getDescription() + " contém:\n" + s;
                    }
                } else {
                    // s += "The " + container.getDescription() + " isn't open.";
                    s += "" + article(t.getName()) + " " + container.getDescription() + " não está abert"+article(t.getName());
                }
            }
        }
        return s;
    ...
    public String lookAt(NounPhrase np) {
        ...
        if (s.isEmpty()) {
            ...
            char caractere_final = t.getName().charAt(t.getName().length()-1);
            char artigo1 = 'a';
            if (caractere_final == 'o')
                artigo1 = 'o';
            char caractere_final2 = th.getName().charAt(th.getName().length()-1);
            char artigo2 = 'a';
            if (caractere_final2 == 'o')
                artigo2 = 'o';
            if (th instanceof ContainerThing) {
                s = artigo1 + " "  + t.getDescription() + " está dentro d" +artigo2 + " " + th.getDescription() + "\n";
            }
            s += t.describe();
        }
        return s;
        ...
        if (t.isTakable()) {
            if (this.inTopLevelList(t)) {
                ...
                if (s.isEmpty()) {
                    ...
                    if (th instanceof ContainerThing) {
                        s = "você pegou "+article(t.getName()) + " "  + t.getDescription() + " d" +article(th.getName()) + " " + th.getDescription();
                    } else {
                        s = t.getDescription() + "foi peg"+article(t.getName()) + ". " ;
                    }
                }
            }
